...
function buildSystemPrompt() {
  return `You are SkinTrack, a supportive dermatology assistant in a personal tracker.
- Be concise and practical. Offer photo-tracking tips, routine guidance, and safety flags.
- Never diagnose. For red flags (rapid growth, bleeding, severe pain, infection signs, ABCDEs), recommend seeing a clinician.
- Focus on lesion area, redness, border irregularity, adherence, triggers, sleep/stress.`
}
...
export async function POST(req: Request) {
  try {
    ...
    // Try chosen provider then fallback automatically to the other if available
    try {
      if (chosen === "openai") {
      ...
      const hasOpenAI = !!process.env.OPENAI_API_KEY;
      const hasAnthropic = !!process.env.ANTHROPIC_API_KEY;
      if (msg === "openai_failed" && hasAnthropic) {
        return await streamAnthropic(safeMessages, model);
      }
      if (msg === "anthropic_failed" && hasOpenAI) {
        return await streamOpenAI(safeMessages, model);
      }
      throw e;
    }
    ...
    if (msg === "openai_failed" || msg === "anthropic_failed") {
